...
async function loadProducts(search = '', category = '') {
    ...
    container.innerHTML = '<div style="text-align: center; width: 100%;">جاري تحميل المنتجات...</div>';
    
    try {
        productsData = await api.getProducts(search, category);
        renderProducts(productsData);
    } catch (err) {
        container.innerHTML = '<div style="text-align: center; width: 100%; color: red;">فشل في تحميل المنتجات! يرجى التأكد من تشغيل السيرفر.</div>';
    }
}
...
function renderProducts(products) {
    ...
    if (products.length === 0) {
        container.innerHTML = '<div style="text-align: center; width: 100%; grid-column: 1/-1;">لا توجد منتجات مطابقة.</div>';
        return;
    }
